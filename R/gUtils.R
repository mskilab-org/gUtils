...
gr.collapse = function(gr, pad = 1)
{
    ...
    strgr = as.character(strand(gr))
    m[which((strgr[-length(gr)] == '+' & (start(gr)[-length(gr)] > start(gr)[-1])) |
            (strgr[-length(gr)] == '-' & (end(gr)[-length(gr)] < end(gr)[-1])))] = FALSE
...
gr_construct_by = function(x, by = NULL, na.seqlevels = TRUE) {
    ...
    this.sep1 = gUtils::BY.SEP1
    this.sep2 = gUtils::BY.SEP2
